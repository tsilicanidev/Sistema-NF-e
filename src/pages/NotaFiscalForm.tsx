...
import { gerarChaveNFe, gerarXmlNFe, validarXmlComXSD } from '../utils/nfeUtils';
...
const NotaFiscalForm = () => {
  ...
  const onSubmit = async (data: NotaFiscalFormData) => {
    try {
      ...
      const isValid = await validarXmlComXSD(xml, 'leiauteNFe_v4.00.xsd');
      if (!isValid) {
        throw new Error('XML da NF-e inválido conforme o XSD. Corrija os dados e tente novamente.');
      }
