...
const ConfiguracaoEmissor: React.FC = () => {
  const { emissor, carregando, salvarEmissor } = useEmissor();
  ...
  const onSubmit = async (data: Emissor) => {
    try {
      ...
      // Se não existir um emissor, cria um novo ID
      if (!data.id) {
        data.id = uuidv4();
      }
      
      await salvarEmissor(data);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          disabled={salvando}
          className="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-md transition-colors disabled:bg-neutral-300 disabled:cursor-not-allowed"
        >
          <Save size={18} />
